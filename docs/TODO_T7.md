...
Actualizado: 9 de septiembre de 2026. T01–T12 completadas; T13 es la siguiente tarea.
...
La siguiente tarea de implementación es T13: calcular por separado área de cómputo, áreas útiles y terraza; resolver 76,66 frente a 76,80 m² y superficie edificada frente a útil sin distorsionar la planta. Usar el contorno T11 y la transformación T10; los contornos funcionales T12 de baño, logia y franja técnica no contienen un cómputo de áreas. Distinguir caras exteriores, interiores, ejes medianeros, umbrales, huecos técnicos y criterio documental; no sumar localizadores de muebles o tramas como superficies independientes. El encabezado p20 dice 76,66 m² EDIFICADO y 0,00 m² TERRAZA, mientras la planta nativa rotula 76,80 m²; la solicitud adopta 76,66 m² interiores sin terraza. Mantener el contraste documental explícito hasta comprobar el criterio que lo explica. La cara exterior superior está recortada y el contorno exterior de la terraza no está acreditado; presentar sensibilidad y supuestos antes de concluir una coincidencia, sin forzar escalas o completar dimensiones inventadas. Reutilizar `architecture/plan/t11-registro.json`, `architecture/topology/t12-topologia.json` y las referencias intactas de `references/derived/p20-t7/`. Si es necesario corregir un borde, modificar su fuente T11, regenerar y repetir los controles dependientes. T12 confirmó un baño longitudinal y logia separada; identidades de EQ01/LG02, detalles de instalaciones, apoyos y alturas continúan pendientes en sus tareas específicas. La raíz Git es `simulador-t7/` y `origin` apunta al repositorio privado `BenjaAranda/DepartamentoT7`. El seguimiento de E02 corresponde a la issue #3; actualizar esta lista, sincronizar casillas y subir la revisión con evidencia. El sitio de `web/.openai/hosting.json` permanece privado y sin publicar; el PDF original está en la carpeta superior. Si Unreal sigue instalándose, continuar por tareas independientes y conservar E08 pendiente.
