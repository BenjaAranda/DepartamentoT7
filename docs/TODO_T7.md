...
Actualizado: 9 de septiembre de 2026. T01–T11 completadas; T12 es la siguiente tarea.
...
La siguiente tarea de implementación es T12: confirmar baño longitudinal, ducha accesible al fondo y separación de logia; identificar la función de los símbolos de cabecera, sanitarios, achurados y volumen técnico. Reutilizar `architecture/plan/t11-registro.json` y las referencias intactas de `references/derived/p20-t7/`; comparar planta e isométrica. Las caras, vanos y bisagras usan exclusivamente la transformación T10. No dividir el baño por tramas, líneas de ducha o equipos ni crear un paso hacia logia sin evidencia. Si la lectura funcional exige corregir un borde, modificar `architecture/plan/t11-trazado.json`, regenerar con `scripts/build-t11-plan.py` y repetir los controles afectados. Conservar los pendientes explícitos de T11: borde superior recortado, celosía probable sin apertura demostrada, cotas nominales y marcos no confirmados, discrepancia de 1 px en P06 y adaptación del armario D3 a dos hojas en T37. T13 resolverá 76,66/76,80 m² y superficie edificada/útil sin distorsionar el perímetro; T14 documentará alturas. La raíz Git es `simulador-t7/` y `origin` apunta al repositorio privado `BenjaAranda/los-altos-algarrobo-t7`. El seguimiento de E02 corresponde a la issue #3; actualizar esta lista, sincronizar sus casillas y subir la revisión con evidencia. Reutilizar el sitio registrado en `web/.openai/hosting.json`, privado y sin publicar. El PDF original permanece en la carpeta superior. Si Unreal sigue instalándose, continuar por tareas independientes y conservar E08 pendiente.
