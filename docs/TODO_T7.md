...
Actualizado: 9 de septiembre de 2026. T01–T09 completadas; T10 es la siguiente tarea.
...
La siguiente tarea de implementación es T10: fijar origen, ejes y escala mediante referencias horizontales y verticales, reservando puntos independientes de comprobación. Partir del inventario `architecture/measurements/` y de las imágenes intactas de `references/derived/p20-t7/`. Elegir extremos geométricos de las cotas candidatas, no las cajas de recorte de texto; contrastar la propuesta de cm para las cifras locales y registrar desviaciones e incertidumbre. Usar los círculos como controles secundarios, no deducir dimensiones de muebles ni forzar la escala para obtener 76,66 m². Los ocho rótulos de 15 cm no fijan el espesor de los demás tabiques; los códigos F y las dimensiones genéricas de p23-p24 no sustituyen datos del T7. Conservar los pendientes de lectura T09 y los 12 pendientes de interpretación T08 hasta resolverlos en sus tareas. La raíz Git es `simulador-t7/` y `origin` apunta al repositorio privado `BenjaAranda/los-altos-algarrobo-t7`. El seguimiento de E02 corresponde a la issue #3; actualizar esta lista, sincronizar sus casillas y subir la revisión con evidencia. Reutilizar el sitio registrado en `web/.openai/hosting.json`, privado y sin publicar. El PDF original permanece en la carpeta superior. El despiece exacto, las alturas y las superficies siguen pendientes en E02. Si Unreal sigue instalándose, continuar por tareas independientes y conservar E08 pendiente.
